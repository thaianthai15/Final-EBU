    ...
    <div className="min-h-screen bg-white font-sans text-[#1a1a1a]">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
            ...
            <div className="text-center lg:text-left z-10">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
                <button className="bg-[#4C8787] text-white px-8 py-3 rounded-full font-bold text-[16px] hover:bg-[#e6004a] transition-colors">
                  Sign up
                </button>
        ...
        <div className="max-w-[1400px] mx-auto px-5 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          {[
            { 
              ...
              desc: <>and still have budget for your business ideas due to <strong>low monthly prices</strong></>,
              img: Pic34
            },
            { 
              title: "Stay agile in turbulent times", 
              desc: <>by adjusting rentals to your current needs thanks to <strong>flexible rental periods</strong></>,
              img: Pic35
            },
            { 
              title: "If an asset breaks, we'll cover up to 80% of the repair costs", 
              small: "GIMO CARE",
              img: Pic36
            },
            { 
            ...
            <div key={idx} className="bg-[#f9f9f9] rounded-xl p-8 flex flex-col justify-between hover:-translate-y-1 hover:shadow-lg transition-all duration-300 min-h-[350px]">
              <div>
                {item.small && <small className="block text-[#ff0052] font-bold mb-2 uppercase text-sm">{item.small}</small>}
                <h3 className="text-[22px] font-bold mb-3 leading-tight">{item.title}</h3>
                {item.desc && <p className="text-[16px] text-[#555] leading-relaxed">{item.desc}</p>}
              </div>
              <img src={item.img} alt={`prop-${idx}`} className="w-full max-w-[200px] self-end mt-4 rounded-lg" />
            </div>
          ))}
      ...
      <section className="py-10 bg-white">
        <div className="max-w-[1200px] mx-auto px-5">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              { title: "Tip-top tech", text: "We guarantee you tech in great condition. Damaged devices don't come in Gimo boxes.", img: Pic30 },
              { title: "Put to the test", text: "After each product is returned, it goes through a multi-process quality check. Only the best go out again.", img: Pic29 },
              { title: "Squeaky clean", text: "We scrub your devices inside and out: erasing all data and cleaning every groove.", img: Pic28 }
            ].map((item, idx) => (
              <div key={idx} className="bg-[#f9f9f9] rounded-xl p-8 flex flex-col justify-between hover:-translate-y-1 hover:shadow-lg transition-all duration-300 min-h-[300px] text-center">
                <div className="mb-4">
                  <h4 className="text-[20px] font-bold mb-2">{item.title}</h4>
                  <p className="text-[14px] text-[#555] leading-relaxed max-w-[250px] mx-auto">{item.text}</p>
                </div>
                <img src={item.img} alt={`promise-${idx}`} className="mx-auto max-w-full h-auto object-contain" />
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-[1200px] mx-auto bg-[#f9f9f9] rounded-[40px] p-8 md:p-16 text-center">
          <small className="block text-[#8a8a8a] font-bold tracking-widest uppercase mb-4 text-[12px]">Renting helps most businesses</small>
          <h2 className="text-[32px] md:text-[38px] font-extrabold mb-12">What kind of company do you work for?</h2>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
            {[
              { type: "Freelancer", img: Pic39 },
              { type: "Startup", prefix: "I work for a (growing)", img: Pic40 },
              { type: "Corporation", prefix: "I work for an established", img: Pic41 },
            ].map((item, idx) => (
              <div key={idx} className="relative rounded-2xl overflow-hidden shadow-lg h-[420px] group">
          ...
          <div>
            <h2 className="text-[20px] font-bold mb-8 text-center">Companies that trust Gimo Business</h2>
            <div className="flex flex-wrap justify-center gap-10 items-center opacity-70">
              {/* Replace with your SVGs */}
              <img src={Allianz} alt="Allianz" className="h-8 md:h-10 hover:opacity-100 transition-opacity" />
              <img src={Bosch} alt="Bosch" className="h-8 md:h-10 hover:opacity-100 transition-opacity" />
              <img src={Deutsche} alt="Telekom" className="h-8 md:h-10 hover:opacity-100 transition-opacity" />
              <img src={DeutscheRentenversicherung} alt="DRV" className="h-8 md:h-10 hover:opacity-100 transition-opacity" />
              <img src={Flink} alt="Flink" className="h-8 md:h-10 hover:opacity-100 transition-opacity" />
              <img src={Toogoodtogo} alt="Toogoodtogo" className="h-8 md:h-10 hover:opacity-100 transition-opacity" />
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 bg-white text-center">
        <div className="max-w-[1200px] mx-auto px-5">
          <h2 className="text-[30px] font-extrabold mb-10">Choose from more than 3,000 products</h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4">
            {[
              ...
              <div key={idx} className="bg-[#f7f7f7] rounded-lg p-4 flex flex-col items-center justify-center min-h-[160px] cursor-pointer hover:bg-[#eee] hover:shadow-md transition-all">
                <img src={cat.img} alt={cat.name} className="w-[80px] h-[80px] object-contain mb-3" />
                <div className="text-[13px] font-semibold text-[#555]">{cat.name}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-10 bg-white">
        <div className="max-w-[1200px] mx-auto bg-[#f9f9f9] rounded-[40px] p-10 md:p-16 text-center">
          <small className="block text-[#555] font-bold uppercase mb-2">Introducing</small>
          <h2 className="text-[32px] font-extrabold mb-2">Gimo Business Premium</h2>
          <p className="text-[18px] text-[#555] mb-6">Manage all your tech in one tool</p>
            ...
            {[
              ...
              <div key={idx} className="flex flex-col items-center">
                <div className="relative mb-6">
                  ...
                  <img src={item.img} alt={`feature-${idx}`} className="w-full max-w-[250px] relative z-10" />
                </div>
                <p className="text-[18px] font-bold text-[#1a1a1a]">{item.text}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-[1200px] mx-auto px-5">
          <div className="flex flex-col md:flex-row items-center gap-10 lg:gap-20">
            <div className="flex-1">
              <img src={Pic38} alt="Circular Economy" className="w-full rounded-xl shadow-md" />
            </div>
            <div className="flex-1 text-center md:text-left">
              <h2 className="text-[32px] md:text-[38px] font-extrabold mb-5">Join the circular economy</h2>
              <p className="text-[18px] text-[#555] leading-relaxed mb-6">
                We believe sharing tech will help further our collective goal to leave this planet better than we found it.
              </p>
              <a href="#" className="text-black font-bold hover:opacity-70 text-[16px]">Why is the circular economy so important? →</a>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="flex flex-col md:flex-row items-center gap-10">
            ...
            <div className="flex-1 text-center md:text-left md:pl-10">
              ...
              <p className="text-[16px] text-[#555] leading-relaxed mb-8">
                Rent your tech worry-free with Gimo Care. Repair service fees will apply. The amount will depend on your Gimo Care option.
              </p>
              <button className="bg-[#4C8787] text-white px-8 py-3 rounded-full font-bold text-[16px] hover:bg-[#e6004a] transition-colors">
                Learn more
              </button>
      ...
      {/* --- SECTION 9: CTA BOTTOM --- */}
      <section className="py-10 bg-white">
        <div className="max-w-[1200px] mx-auto bg-[#f9f9f9] rounded-[50px] p-16 text-center">
          <h2 className="text-[32px] md:text-[40px] font-extrabold mb-2">Ready to get started?</h2>
          <h3 className="text-[20px] md:text-[24px] font-semibold text-[#555] mb-8">Register with Gimo Business</h3>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button className="bg-[#4C8787] text-white px-8 py-3 rounded-full font-bold text-[16px] hover:bg-[#e6004a] transition-colors">
              Sign up
            </button>
            <button className="bg-white text-[#1a1a1a] border border-[#ccc] px-8 py-3 rounded-full font-bold text-[16px] hover:bg-[#f0f0f0] transition-colors">
              Find tech
            </button>
          </div>
        </div>
